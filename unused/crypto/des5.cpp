...
//      des5.cpp
//      Differential cryptanalysis of 5 Round DES
//
//	John Manferdelli
...
void des5Diff(unsigned* input, unsigned* inDiff, unsigned* outputD, bool fReverse= false, bool fVeryNoisy=false) 
{
    int         i, j, k;
...
void des5(unsigned* input, unsigned* output, bool fReverse= false, bool fVeryNoisy=false) 
{
    int         i, j, k;
    byte        fout[32]; 
    byte        dbits[64];
...
void AnalyseDes5Diff(int in, unsigned* inDiff, unsigned* outDiff, bool fReverse= false, bool fVeryNoisy=false) 
{
    int         i, j, k;
    int         nb= 0;
    ...
    for(;;) {
        ...
        des5(inblock1, outblock1, false, false);
        des5(inblock2, outblock2, false, false);
        ...
        for(tkey=0; tkey<64; tkey++) {
            ...
            if(fVeryNoisy) {
                prbit((char*)"  bKey:", bKey, 6, 8); prbit((char*)", S2In1:", S2In1, 6, 8); 
                prbit((char*)", result1:", result1, 4, 6);  
                prbit((char*)", S2In2:", S2In2, 6, 8); prbit((char*)", result2:", result2, 4, 6); 
                prbit((char*)", resultD:", resultD, 4, 6);
                printf((char*)"\n");
            }
...

{
    ...
    if(argn<2) {
        printf((char*)"des5 [-GenRight|-DoAttack] [-NumRight n] -k 01234567 89abcdef input output\n");
        return 1;
    }
    ...
    if(fGenRight) {
        ...
        printf((char*)"des5, Generating right pairs, Input: %s, Output: %s, Key: %08x%08x\n\n", szIn, szOut, key[0], key[1]);
        ...
        for(;;) {
            ...
            des5Diff(inblock1, inDiff, outputD, false, fVeryNoisy);
    ...
    // Use right pairs to find key in round 5
    if(fDoAttack) {
        ...
        printf((char*)"des5, Analyzing right pairs, Input: %s, Key: %08x%08x\n\n", 
		szIn, key[0], key[1]);

        compkeys(key, true); printf((char*)"\n");

        AnalyseDes5Diff(in, inDiff, outDiff, false, fVeryNoisy); 
