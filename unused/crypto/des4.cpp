...
void des4Diff(unsigned* input, unsigned* inDiff, unsigned* outputD, bool fReverse= false, bool fVeryNoisy=false) 
{
    int         i, j, k;
    ...
    for(i=1;i<=4;i++) {
        if(!fReverse) {
            ...
            f(5-i, fout1, &dbits1[32]);
            f(5-i, fout2, &dbits2[32]);
        }
        if(i!=4) {
            for(j=0;j<32;j++) {
                k= dbits1[j];
...
void des4(unsigned* input, unsigned* output, bool fReverse= false, bool fVeryNoisy=false) 
{
    int         i, j, k;
    byte        fout[32]; 
    byte        dbits[64];
...
void AnalyseDes4Diff(int in, unsigned* inDiff, unsigned* outDiff, bool fReverse= false, 
                     bool fVeryNoisy=false, int iNum=16) 
{
    int         i, j, k;
    int         nb= 0;
    int         iBox;
    unsigned    uMask= 0x00808202;
    unsigned    inblock[2], outputD[2];
    unsigned    inblock1[2], outblock1[2];
    unsigned    inblock2[2], outblock2[2];
    unsigned    uRound4DiffIn, uRound4DiffOut;
    byte        iP[32];
    ...
    for(;;) {
        ...
        des4(inblock1, outblock1, false, fVeryNoisy);
        des4(inblock2, outblock2, false, fVeryNoisy);

        outputD[0]= outblock1[0]^outblock2[0];
        outputD[1]= outblock1[1]^outblock2[1];

        printf((char*)"\nDes4 input (%08x %08x, %08x %08x), output(%08x %08x, %08x %08x)\n", 
                inblock1[0], inblock1[1], inblock2[0], inblock2[1],
                outblock1[0], outblock1[1], outblock2[0], outblock2[1]);
        printf((char*)"              input  difference: %08x %08x, ", inDiff[0], inDiff[1]);
        printf((char*)" output difference: %08x %08x\n", outputD[0], outputD[1]);

        uRound4DiffIn= outputD[1];
        uRound4DiffOut= outputD[0];     // right except at uMask
        printf((char*)"Differential: %08x -->%08x, ", uRound4DiffIn, uRound4DiffOut);
        printf((char*)"masked bits: %08x\n", uRound4DiffOut&uMask);

        tobits(uRound4DiffIn, allSDiffIn);
        tobits(uRound4DiffOut, allSDiffOut);
        tobits(uMask, allSOutMask);
        ...
        if(fVeryNoisy) {
            prbit((char*)"Inverted differential:", SInvertedDifferential, 32, 8); printf((char*)"\n\n");
        }

        for(iBox=0; iBox<8; iBox++) {

            // For each S-box try keys to see if the output differential holds except for the mask
            // Fill S-box inputs and output test bits
            for(j=0;j<6;j++) {
                SIn1[j]= allSIn1[eb[iBox*6+j]-1];
                SIn2[j]= allSIn2[eb[iBox*6+j]-1];
            }
            for(j=0;j<4;j++) {
                SOutMask[j]= allSOutMask[iBox*4+j];
                SCmpDiff[j]= SInvertedDifferential[iBox*4+j];
            }
            ...
            for(tkey=0; tkey<64; tkey++) {
                ...
                SingleBoxf(iBox, bKey, SOut1, SIn1);
                SingleBoxf(iBox, bKey, SOut2, SIn2);
    
                fMatch= true;
                for(j=0;j<4;j++) {
                    SDiff[j]= SOut1[j]^SOut2[j];
                    if(SOutMask[j]==0 && SDiff[j]!=SCmpDiff[j])
                        fMatch= false;
                }

                if(fVeryNoisy) {
                    printf("Box %d, ",iBox+1);
                    prbit((char*)"bKey:", bKey, 6, 8); 
                    prbit((char*)", SIn1:", SIn1, 6, 8); 
                    prbit((char*)", SOut1:", SOut1, 6, 8); 
                    prbit((char*)", SIn2:", SIn2, 6, 8); 
                    prbit((char*)", SOut2:", SOut2, 6, 8); 
                    prbit((char*)", SDiff:", SDiff, 6, 8); 
                    prbit((char*)", SCmpDiff:", SCmpDiff, 4, 8); 
                    prbit((char*)", SOutMask:", SOutMask, 4, 8); 
                    printf((char*)"\n");
                }
    ...
    for(iBox=0;iBox<8;iBox++) {
        printf((char*)"Box %d, Potential keys:\n", iBox+1);
        max= 0;
        for(j=0;j<64;j++) {
            if(iKey[iBox*64+j]>max)
                max= iKey[iBox*64+j];
        }
        for(j=0;j<64;j++) {
            if(iKey[iBox*64+j]==max)
...

{
    ...
    if(argn<2) {
        printf((char*)"des4 -DoAttack [-Num n] -k 01234567 89abcdef input output\n");
        return 1;
    }

    inDiff[0]=  0x20000000;
    inDiff[1]=  0x00000000;
    outDiff[0]= 0x00000000;
    outDiff[1]= 0x00000000;
    ...
    if(fDoAttack) {
        ...
        printf((char*)"des4, Input: %s, Key: %08x%08x, %d trials\n\n", 
               szIn, key[0], key[1], iNum);

        compkeys(key, true); printf((char*)"\n");

        AnalyseDes4Diff(in, inDiff, outDiff, false, fNoisy, iNum); 
