...
void Uint64AddWithCarryStep(uint64_t a, uint64_t b, uint64_t carry_in, 
  ...
  :: [result]"g"(result), [carry_out]"g"(carry_out),
     [a]"g"(a),[b]"g"(b), [carry_in]"g"(carry_in)
  :"cc", "memory", "%rax", "%rbx", "%rcx", "%rdx");
}
...
void Uint64SubWithBorrowStep(uint64_t a, uint64_t b, uint64_t borrow_in, 
  ...
  asm volatile (
    ...
    "2:\n"
    "\tmovq   %%rax,(%%rcx)\n"
  :: [result]"g"(result), [borrow_out]"g"(borrow_out),
...
void Uint64MultWithCarryStep(uint64_t a, uint64_t b, 
                             uint64_t carry1, uint64_t carry2, 
                             uint64_t* result, uint64_t* carry_out) {
  asm volatile (
    "\tmovq   %[result], %%rcx\n"
    "\tmovq   %[carry_out], %%rbx\n"
...
// a+= b
int DigitArrayAddTo(int capacity_a, int size_a, uint64_t* a, int 
                    size_b, uint64_t* b) {
...
// a-= b
int DigitArraySubFrom(int capacity_a, int size_a, uint64_t* a, 
                      int size_b, uint64_t* b) {
  if(size_a<size_b)
    return -1;
...
// printf("DigitArrayShortDivisionAlgorithm %ld/%ld\n", 
  ...
  asm volatile (
    ...
    "\tmovq   %[r],%%rbx\n"
    "\tmovq   %%rdx, (%%rbx)\n"
  :: [r]"g"(r), [len_a]"g"(len_a), [a_high]"g"(a_high), 
     [b]"g"(b), [q_high]"g"(q_high)
  :"cc", "memory", "%rax", "%rbx", "%rcx", "%rdx", "%r8");
...
bool DigitArrayDivisionAlgorithm(int size_a, uint64_t* a, int size_b, uint64_t* b, 
                    int* size_q, uint64_t* q, int* size_r, uint64_t* r) {
  int         real_size_a= DigitArrayComputedSize(size_a, a);
  int         real_size_b= DigitArrayComputedSize(size_b, b);
  ...
  if(real_size_b==1) {
    ...
    return  DigitArrayShortDivisionAlgorithm(real_size_a, a, b[0],
                        size_q, q, &r[0]);
  }
  ...
  for(i= hi_quotient_digit;i>=0; i--) {
    ...
    n= real_size_b+i;
    if(n==real_size_a) {
      EstimateQuotient(0ULL, r[n-1], r[n-2],
                       b[real_size_b-1], b[real_size_b-2], 
                       &x);
    } else if(n<2) {
     EstimateQuotient(r[n], r[n-1], 0ULL,
                      b[real_size_b-1], b[real_size_b-2], 
                      &x);
    } else {
      EstimateQuotient(r[n], r[n-1], r[n-2],
                      b[real_size_b-1], b[real_size_b-2], 
                      &x);
    }
    for(;;) {
...
bool DigitArrayConvertToDecimal(int size_a, uint64_t* a, int* size_s, char* s) {
  ...
  if(!DigitArrayCopy(real_size_a, a, real_size_a, t[0]))
    return false;
  for(;;) {
    m= DigitArrayComputedSize(real_size_a, t[src]);
    DigitArrayZeroNum(real_size_a, t[dst]);
    if(!DigitArrayShortDivisionAlgorithm(real_size_a, t[src], ten,
                        &m, t[dst], &r))
      return false;
