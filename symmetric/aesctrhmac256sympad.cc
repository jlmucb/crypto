...
bool  AesCtrHmac256Sympad::FinalPlainIn(int size_in, byte* in, int* size_out, byte* out) {
...
bool  AesCtrHmac256Sympad::FinalCipherIn(int size_in, byte* in, int* size_out, byte* out) {
...
bool  AesCtrHmac256Sympad::ProcessFinalInput(int size_in, byte* in, int* size_out, byte* out) {
...
bool  AesCtrHmac256Sympad::PlainIn(int size_in, byte* in, int* size_out, byte* out) {
  int   n;
  int   num_iv= 0;
  int   num_out= 0;
...
bool  AesCtrHmac256Sympad::CipherIn(int size_in, byte* in, int* size_out, byte* out) {
  int num_out= 0;
  int num_iv= 0;
...
bool  AesCtrHmac256Sympad::ProcessInput(int size_in, byte* in, 
...
int   AesCtrHmac256Sympad::DecryptInputQuantum() {
  return Aes::BLOCKBYTESIZE;
}

int   AesCtrHmac256Sympad::EncryptInputQuantum() {
  return 1;
}

int   AesCtrHmac256Sympad::MaxAdditionalOutput() {
  return Aes::BLOCKBYTESIZE;
}
...
int   AesCtrHmac256Sympad::MinimumFinalEncryptIn() {
  return 1;
}
...
bool  AesCtrHmac256Sympad::MessageValid() {
...
bool AesCtrHmac256Sympad::MakeScheme(const char* id, int num_bits, 
  ...
  return Init(num_bits/NBITSINBYTE, enc_key, num_bits/NBITSINBYTE, 
                int_key, 4, nonce, 8, iv, false);
}
