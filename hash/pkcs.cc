...
bool PkcsVerify(const char* hash_alg, byte* hash, int in_size, byte* in) {
  ...
  if(strcmp(hash_alg, "sha-1")==0) {
    ...
    if(in[m++]!=0x00)
      return false;
    if(memcmp(&in[m], sha1_digest_info, n)!=0)
      return false;
    m+= n;
    if(memcmp(&in[m], hash, 20)!=0)
      return false;
    return true;
  } else if(strcmp(hash_alg, "sha-256")==0) {
    n= sizeof(sha256_digest_info);
    ...
    if(in[m++]!=0x00)
      return false;
    if(memcmp(&in[m], sha256_digest_info, n)!=0)
      return false;
    m+= n;
    if(memcmp(&in[m], hash, 32)!=0)
      return false;
    return true;
  } else if(strcmp(hash_alg, "sha-512")==0) {
    n= sizeof(sha1_digest_info);
    ...
    if(in[m++]!=0x00)
      return false;
    if(memcmp(&in[m], sha512_digest_info, n)!=0)
      return false;
    m+= n;
    if(memcmp(&in[m], hash, 64)!=0)
      return false;
    return true;
  } else {
    return false;
...
bool PkcsEmbed(int in_size, byte* in, int out_size, byte* out) {
  ...
  if(!GetCryptoRand(pad_size*NBITSINBYTE, &out[m]))
    return false;
...
bool PkcsExtract(int in_size, byte* in, int* out_size, byte* out) {
  ...
  if(in[m++]!=0x02)
    return false;
  while(size_pad<(in_size-3) && in[m+size_pad]!=0)
    size_pad++;
