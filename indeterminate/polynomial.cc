...
Polynomial::Polynomial(int size_num, int num_c) {
  ...
  c_ = new BigNum* [num_c];
  for (i = 0; i < num_c; i++) c_[i] = new BigNum(size_num);
  size_num_ = size_num;
  num_c_ = num_c;
}
...
int Polynomial::Degree() {
  ...
  for (i = (num_c_ - 1); i >= 0; i--) {
    if (!c_[i]->IsZero()) return i;
  }
  return -1;
}

bool Polynomial::IsZero() { return Degree() == (-1); }
...
bool Polynomial::CopyTo(Polynomial& a) {
  ...
  if (a.num_c_ <= Degree()) return false;
  ZeroPoly(a);
  for (i = Degree(); i >= 0; i--) {
    c_[i]->CopyTo(*a.c_[i]);
  }
...
bool Polynomial::CopyFrom(Polynomial& a) { return a.CopyTo(*this); }
...
bool PolyIsEqual(Polynomial& a, Polynomial& b) {
  ...
  for (i = (a.num_c_ - 1); i >= 0; i--) {
    if (BigCompare(*a.c_[i], *b.c_[i]) != 0) return false;
  }
  return true;
}
...
bool PolyAdd(Polynomial& a, Polynomial& b, Polynomial& c) {
  ...
  if (b.Degree() > a.Degree()) return PolyAdd(b, a, c);
  int i;

  ZeroPoly(c);
  for (i = 0; i <= b.Degree(); i++) {
    if (!BigModAdd(*a.c_[i], *b.c_[i], *a.m_, *c.c_[i])) return false;
  }
  for (; i <= a.Degree(); i++) {
    if (!c.c_[i]->CopyFrom(*a.c_[i])) return false;
  }
  return true;
}

bool PolySub(Polynomial& a, Polynomial& b, Polynomial& c) {
  if (max(a.Degree(), b.Degree()) >= c.num_c_) return false;
  ...
  if (b.Degree() <= a.Degree()) {
    for (i = 0; i <= b.Degree(); i++) {
      if (!BigModSub(*a.c_[i], *b.c_[i], *a.m_, *c.c_[i])) return false;
    }
    for (; i <= a.Degree(); i++) {
      if (!c.c_[i]->CopyFrom(*a.c_[i])) return false;
    }
    ...
    for (; i <= b.Degree(); i++) {
      if (!BigModSub(Big_Zero, *b.c_[i], *a.m_, *c.c_[i])) return false;
    }
  }
  return true;
}
...
bool PolyMult(Polynomial& a, Polynomial& b, Polynomial& c) {
  if ((a.Degree() + b.Degree()) >= c.num_c_) return false;
  ...
  for (i = 0; i <= a.Degree(); i++) {
    for (j = 0; j <= b.Degree(); j++) {
      k = i + j;
      if (!BigModMult(*a.c_[i], *b.c_[j], *a.m_, t)) return false;
      if (!BigModAdd(t, *c.c_[k], *a.m_, r)) return false;
      r.CopyTo(*c.c_[k]);
    }
  }
  return true;
...
bool ZeroPoly(Polynomial& a) {
  ...
  for (i = 0; i < a.num_c_; i++) a.c_[i]->ZeroNum();
  return true;
}
...
bool PolyEuclid(Polynomial& a, Polynomial& b, Polynomial& q, Polynomial& r) {
  if (q.num_c_ <= (a.Degree() - b.Degree())) return false;
  if (r.num_c_ <= b.Degree()) return false;
  Polynomial t(a.size_num_, a.num_c_, *a.m_);
  Polynomial s(a.size_num_, a.num_c_, *a.m_);

  if (!t.CopyFrom(a)) return false;
  int deg_t = t.Degree();
  int deg_b = b.Degree();
  int cur_q;
  if (deg_t < deg_b) {
    if (!r.CopyFrom(a)) return true;
  }
  ...
  if (!BigModInv(*b.c_[deg_b], *a.m_, leading_b_inv)) return false;
  while (deg_t >= deg_b) {
    cur_q = deg_t - deg_b;
    if (!BigModMult(*t.c_[deg_t], leading_b_inv, *a.m_, tn)) return false;
...
bool PolyExtendedGcd(Polynomial& a, Polynomial& b, Polynomial& x, Polynomial& y,
  ...
  for (i = 0; i < 3; i++) {
    if (a_coeff[i] != nullptr) delete a_coeff[i];
    a_coeff[i] = nullptr;
    if (b_coeff[i] != nullptr) delete b_coeff[i];
    b_coeff[i] = nullptr;
...
bool MultiplyPolyByMonomial(Polynomial& a, int d, BigNum& n, Polynomial& r) {
  ...
  if (r.num_c_ <= (a.Degree() + d)) return false;
  ZeroPoly(r);
  for (i = a.Degree(); i >= 0; i--) {
    if (!BigModMult(*a.c_[i], n, *a.m_, *r.c_[i + d])) return false;
  }
  return true;
}
