...
bool kyber_kem_keygen(int g, kyber_parameters& p, int* kem_ek_len, byte_t* kem_ek,
  ...
  if (*kem_dk_len < len) {
    printf("kyber_kem_keygen: dk_kem buffer too small\n");
    return false;
  }
  *kem_dk_len = len;
#if 1
  printf("ek (%d) :\n", ek_PKE_len);
  print_bytes(ek_PKE_len, kem_ek);
  printf("\n");
  printf("dk_PKE (%d) :\n", dk_PKE_len);
  print_bytes(dk_PKE_len, dk_PKE);
  printf("\n");
  printf("h (%d) :\n", 32);
  print_bytes(32, ph);
  printf("\n");
