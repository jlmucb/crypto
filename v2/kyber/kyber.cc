...
bool kyber_keygen(int g, kyber_parameters& p, int* ek_len, byte* ek,
  ...
  if (!ntt_module_apply_array(g, A_ntt, s_ntt, &r_ntt)) {
    printf("kyber_keygen:  ntt_module_apply_array failed\n");
    return false;
  }
...
bool kyber_encrypt(int g, kyber_parameters& p, int ek_len, byte* ek,
  ...
  for (int i = 0; i < p.k_; i++) {
    ...
    if (!byte_encode_from_vector(p.du_, 256, compressed_u.c_[i]->c_, pp)) {
        return false;
    }
    pp += p.du_ * 32;
  }
...
bool kyber_decrypt(int g, kyber_parameters& p, int dk_len, byte* dk,
  ...
  for (int i = 0; i < p.k_; i++) {
    ...
    if (!ntt_module_vector_dot_product_first_transposed(s_transposed, u_ntt, products.c_[i])) {
        return false;
    }
  ...
  if (!byte_encode_from_vector(1, 256, m_vec.c_, m)) {
    return false;
  }
  return true;
}
