...
RationalPoly::RationalPoly(int size_num, int num_c, BigNum& c, Polynomial& t,
                           Polynomial& b) {
  top_ = new Polynomial(size_num, num_c, c);
  bot_ = new Polynomial(size_num, num_c, c);
  top_->CopyFrom(t);
...
int RationalPoly::Degree() {
  ...
  if (deg_num > deg_den) return deg_num;
  return deg_den;
}

bool RationalPoly::IsZero() {
  if (top_->IsZero() && !bot_->IsZero()) return true;
  return false;
}

bool RationalPoly::IsOne() {
  if (top_->IsOne() && bot_->IsOne()) return true;
  return false;
}

bool RationalPoly::CopyTo(RationalPoly& a) {
  if (!top_->CopyTo(*a.top_)) return false;
  if (!bot_->CopyTo(*a.bot_)) return false;
  return true;
}

bool RationalPoly::CopyFrom(RationalPoly& a) { return a.CopyTo(*this); }
...
bool RationalIsEqual(RationalPoly& a, RationalPoly& b) {
  ...
  if (!b.Reduce()) return false;
  if (!PolyMult(*a.top_, *b.bot_, x)) return false;
  if (!PolyMult(*b.top_, *a.bot_, y)) return false;
...
bool RationalAdd(RationalPoly& a, RationalPoly& b, RationalPoly& c) {
  ...
  if (!PolyMult(*a.bot_, *b.bot_, *c.bot_)) return false;
  if (!PolyMult(*a.top_, *b.bot_, t1)) return false;
  if (!PolyMult(*b.top_, *a.bot_, t2)) return false;
  if (!PolyAdd(t1, t2, *c.top_)) return false;
  return c.Reduce();
}
...
bool RationalSub(RationalPoly& a, RationalPoly& b, RationalPoly& c) {
  ...
  if (!PolyMult(*a.top_, *b.bot_, t1)) return false;
  if (!PolyMult(*b.top_, *a.bot_, t2)) return false;
  if (!PolySub(t1, t2, *c.top_)) return false;
  return c.Reduce();
}
...
bool RationalPoly::Reduce() {
  ...
  if (bot_->IsZero() || top_->IsZero()) return true;
  if (!PolyExtendedGcd(*top_, *bot_, x, y, g)) return false;
  if (g.Degree() == 0) return true;
  ZeroPoly(x);
  ZeroPoly(y);
  if (!PolyEuclid(*top_, g, x, y)) return false;
  top_->CopyFrom(x);
  ZeroPoly(x);
  ZeroPoly(y);
  if (!PolyEuclid(*bot_, g, x, y)) return false;
  bot_->CopyFrom(x);
  return true;
}

bool RationalMult(RationalPoly& a, RationalPoly& b, RationalPoly& c) {
  if (!PolyMult(*a.top_, *b.top_, *c.top_)) return false;
  if (!PolyMult(*a.bot_, *b.bot_, *c.bot_)) return false;
  return c.Reduce();
}

bool RationalDiv(RationalPoly& a, RationalPoly& b, RationalPoly& c) {
  if (!PolyMult(*a.top_, *b.bot_, *c.top_)) return false;
  if (!PolyMult(*a.bot_, *b.top_, *c.bot_)) return false;
  return c.Reduce();
}

bool ZeroRational(RationalPoly& a) {
  if (!ZeroPoly(*a.top_)) return false;
  if (!OnePoly(*a.bot_)) return false;
  return true;
}

bool OneRational(RationalPoly& a) {
  if (!OnePoly(*a.top_)) return false;
  if (!OnePoly(*a.bot_)) return false;
  return true;
}
